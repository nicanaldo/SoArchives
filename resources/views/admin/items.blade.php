...
<body> 
 ...
 <div class="modal fade" id="deleteModal{{ $item->ProductID }}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
                                  <div class="modal-dialog">
                                      <div class="modal-content">
                                          <div class="modal-header">
                                          ...
                                          <div class="modal-footer">
                                              <form id="delete-form-{{ $item->ProductID }}" action="{{ route('products.destroy', ['product' => $item->ProductID]) }}" method="POST">
                                                  @csrf
                                                  @method('DELETE')
...
<!-- Edit Modal -->
@foreach ($products as $item)
        <div class="modal fade" id="editModalbuyer{{$item->ProductID}}" tabindex="-1" aria-labelledbsy="editModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form method="post" action="{{ route('product.update', ['product' => $item->ProductID]) }}">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="ProductName" class="form-control" id="ProductName" value="{{ $item->ProductName }}">
                            </div>
                            <div class="mb-3">
                                <label for="ProductDescription" class="form-label"> Product Description</label>
                                <input type="text" name="ProductDescription" class="form-control" id="ProductDescription" value="{{ $item->ProductDescription }}">
                            </div>
                            <div class="mb-3">
...
<!-- End Modal edit -->
                    ...
                    <!-- Modal View -->
                @foreach ($products as $item)
                        <div class="modal fade" id="viewModal{{ $item->ProductID}}" tabindex="-1" aria-labelledby="viewModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content" style="font-family: Arial, sans-serif; font-size: 15px;">
                                    ...
                                    <div class="modal-body">
                                        <form method="post" action="{{ route('product.get', ['product' => $item->ProductID]) }}">
                                            @csrf
                                            @method('GET')
                                            
                                                <div class="col-md-12 mb-3">
                                                    <label class="form-label">Product Name</label>
                                                    <input id="userID" type="text" class="form-control border"value="{{ $item->ProductName }}" readonly>
                                                </div>
                                                <div class="col-md-12 mb-3">
                                                    <label class="form-label">Product Description</label>
                                                    <input id="violation" type="text" class="form-control"value="{{ $item->ProductDescription }}" readonly>
                                                </div>
                                                <div class="row">
                                                <div class="col-md-6 mb-3">
                                                    <label class="form-label">Price</label>
                                                    <input id="firstName" type="text" class="form-control" value="{{ $item->Price }}"readonly>
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label class="form-label">Quantity</label>
                                                     <input id="lastName" type="text" class="form-control" value="{{ $item->Quantity }}"readonly>
                                                </div>
                                            </div>
                                            <div class="row">
                                                <div class="col-md-6 mb-3">
    ...
    <div class="main-container"> 
        ...
        <div class="main"> 
    <div class="container">
        <h2 style="margin-left: 8px;">Products</h2>
                ...
                <tr>
                    ...
                    <td>
                        @foreach(explode(',', $item->ProductImage) as $image)
                            <img src="{{ asset('storage/products/' . $item->id . '/' . basename($image)) }}" alt="Product Image" width="100" height="100">
                        @endforeach
                    <td>
                        <button href="#" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#viewModal{{ $item->ProductID }}" style="margin-right: 2px; padding: 0.1rem 0.9rem; font-size: 0.6rem; width: 50px;">View</button>
                        <button href="#" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#editModalbuyer{{ $item->ProductID}}" style="margin-right: 2px; padding: 0.1rem 0.9rem; font-size: 0.6rem; width: 50px;">Edit</button>
                        <button href="#" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $item->ProductID }}" style="margin-right: 2px; padding: 0.1rem 0.10rem; font-size: 0.6rem; width: 50px;">Delete</button>
                    </td>
                </tr>
                @endforeach
            </tbody>
