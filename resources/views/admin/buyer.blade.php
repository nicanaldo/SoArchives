<link href="{{asset('css/maindash.css')}}" rel="stylesheet">
            ...
            <div class="modal fade" id="deleteModal{{ $item->id }}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form id="delete-form-{{ $item->id }}" method="POST" action="{{ route('buyers.destroy', ['user' => $item->id]) }}">
                            @csrf
                            @method('DELETE')
            ...
            <div class="modal fade" id="editModalbuyer{{ $item->id }}" tabindex="-1" aria-labelledbsy="editModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <form method="post" action="{{ route('buyers.update', ['user' => $item->id]) }}">
                            @csrf
                            @method('PUT')
                            <div class="modal-body">
    ...
    <!-- End Modal edit -->
                            ...
                            <div class="modal fade" id="viewModal{{ $item->id }}" tabindex="-1" aria-labelledby="viewModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content" style="font-family: Arial, sans-serif; font-size: 15px;">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="viewModalLabel">Buyer Details</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form method="post" action="{{ route('buyers.get', ['user' => $item->id]) }}">
                                                @csrf
                                                @method('GET')
                                                <div class="row">
        ...
        @if ($errors->any())
            ...
            <div class="main"> 
        <div class="container">
            <h2 style="margin-left: 8px;">Buyer</h2>
                    ...
                    @foreach ($users->reverse() as $item)
                        @if ($item->UserTypeID === 3)
                            <tr>
                                ...
                                <td>
                                <div class="link-wrapper" style="margin-left:50px;">
                                    {{ $item->password }}
                                </div>
                                </td>
                                <td>
                                    @foreach ($buyers as $buyer)
                                        @if ($buyer->user_id === $item->id)
                                            {{ $buyer->Violation }}
                                        @endif
                                    @endforeach
                                </td>
